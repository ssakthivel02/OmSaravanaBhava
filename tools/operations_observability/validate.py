...
def validate_repository(root: Path, mode: str="package") -> dict:
    ...
    report={"release":244,"mode":mode,"status":"PASS" if not failed else "FAIL","checkCount":len(results),"automatedCount":catalog["automatedCount"],"advisoryCount":catalog["advisoryCount"],"failedCount":len(failed),"results":results}
    if mode=="final":
        import subprocess
        parent=subprocess.check_output(["git","rev-parse","HEAD^"],cwd=root,text=True).strip()
        subject=subprocess.check_output(["git","log","-1","--pretty=%s"],cwd=root,text=True).strip()
        if parent!="2e8b4d2d4a47931757423906015e586eafcd26a3" or subject!="Release 244: establish production observability and deployment attestation": report["status"]="FAIL"
    return report

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--root",default=".");ap.add_argument("--mode",choices=["package","final"],default="package");ap.add_argument("--report")
    args=ap.parse_args();report=validate_repository(Path(args.root).resolve(),args.mode)
    if args.report:
        p=Path(args.report);p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(report,indent=2)+"\n")
    print(f"Operations observability: {report['status']} ({report['failedCount']} failed, {report['advisoryCount']} advisory)")
