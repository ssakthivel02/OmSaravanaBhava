...
def validate(*, root: Path, policy: dict, contract: dict, manifest: dict, mode: str, tracked: set[str] | None = None, staged: dict[str, str] | None = None, parent: str | None = None, subject: str | None = None) -> dict:
    ...
    if mode == "staged" and staged is not None:
        ...
        wrong = sorted(p for p in set(expected) & set(staged) if expected[p] != staged[p])
    ...
    if mode == "final":
        if parent != manifest.get("base_commit"):
            findings.append(Finding("commit-parent", "Commit parent is not the approved base."))
        if subject != manifest.get("required_commit_title"):
    ...
    return {
        ...
        "declaredDeletedCount": len(manifest.get("deleted_files", [])),
