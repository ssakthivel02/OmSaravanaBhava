...
def static_findings(root: Path, policy: dict, contract: dict, manifest: dict, tracked: set[str] | None) -> list[Finding]:
    ...
    if manifest.get("release") != 243:
        findings.append(Finding("manifest-release", "Manifest release must be 243."))
    ...
    if not sw.is_file() or "const RELEASE = '243';" not in sw.read_text(encoding="utf-8"):
        findings.append(Finding("service-worker-release", "Service worker is not Release 243.", "service-worker.js"))
