...
const routeCard = route => {
  ...
  [route.category, route.status, route.routeAdditionApplied ? 'New governed route' : ''].filter(Boolean).forEach(value => {
    pills.appendChild(create('span', 'premium-pill', value));
  });
  const heading = create('h3', '', route.titleTa
    ? `${route.titleTa} · ${route.titleEn || ''}`
    : route.titleEn || route.path
  );
  const summary = create('p', '', route.summary || 'Governed public route.');
...
const initialise = async () => {
  ...
  if (!status || !metrics || !grid || !query || !category || !routeStatus || !clear) return;

  const [registryResult, completenessResult, recoveryResult] = await Promise.allSettled([
    loadEffectiveRouteRegistry(),
    fetchJson('/data/content-completeness.json'),
    fetchJson('/data/route-recovery-summary.json')
  ]);
  if (registryResult.status !== 'fulfilled') {
    status.textContent = 'The governed route registry could not be loaded. Use the primary navigation and sitemap.';
    grid.innerHTML = '<article class="premium-card" role="alert">Route directory unavailable.</article>';
    return;
  }
  ...
  metrics.replaceChildren(
    ...
    metric(categories.length, 'Knowledge categories'),
    metric(completeness.canonicalFullSongRecords || 0, 'Canonical full-song records'),
    metric(recovery.uniqueSafeRepairsOnCanonicalPages || 0, 'Safe link repairs queued')
  );
  ...
  const render = () => {
    ...
    const filtered = records.filter(record => {
      ...
      String(left.titleEn || left.path).localeCompare(String(right.titleEn || right.path))
    );
    grid.replaceChildren();
    if (!filtered.length) {
      grid.appendChild(create('article', 'premium-card', 'No governed route matched the selected filters.'));
    } else {
      filtered.forEach(record => grid.appendChild(routeCard(record)));
    }
    status.textContent = `${filtered.length} of ${records.length} routes shown. ${registryStatusMessage(registry)}`;
  };
  ...
  document.documentElement.dataset.routeConsumerRelease = String(CONSUMER_RELEASE);
};
