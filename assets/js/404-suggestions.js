(() => {
  ...
  const tokensFor = value => normalisePath(value)
    ...
    .split(/[^\p{L}\p{N}]+/u)
    .filter(token =>
      token.length > 1 && !['html', 'htm', 'www'].includes(token)
    );
  ...
  const makeSuggestionCard = route => {
    ...
    if (route.status) {
      appendText(article, 'span', route.status, 'pill');
    }
  ...
  const rankRoutes = (routes, currentPath) => {
    ...
    return routes
      ...
      .filter(route =>
        (normalisePath(route.path).replace(/\/+$/, '') || '/') !== current
      )
      .map(route => {
        const title = `${route.titleTa || ''} ${route.titleEn || ''}`
          .toLocaleLowerCase();
        ...
        for (const token of pathTokens) {
          ...
          if (
            pathSegments.has(token) &&
            tokensFor(route.path).includes(token)
          ) score += 4;
        }
        if (
          ['published', 'published-source-linked', 'partial-reviewed',
            'navigation', 'directory'].includes(route.status)
        ) score += 1;
        return {...route, score};
      })
      .sort((left, right) =>
  ...
  const applyExactAlias = async currentPath => {
    try {
      ...
      const match = records.find(
        record => normalisePath(record.path) === currentPath
      );
      ...
      if (notice) {
        notice.textContent = `Verified legacy address found. Redirecting to ${target}`;
      }
      ...
      console.warn('[OmSaravanaBhava] Exact alias registry unavailable', error);
      return false;
    }
  };

  const loadGovernedRoutes = async () => {
    try {
      ...
      console.warn(
        '[OmSaravanaBhava] Effective registry unavailable for 404',
        error
      );
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (searchInput) {
      searchInput.value = tokensFor(currentPath).slice(0, 4).join(' ');
    }
    ...
    try {
      ...
      const selected = (
        positive.length
          ? positive
          : fallbackPaths
            .map(path => ranked.find(
              route => normalisePath(route.path) === path
            ))
            .filter(Boolean)
      ).slice(0, 6);
      ...
      if (selected.length) {
        ...
        appendText(
          grid,
          'div',
          'Open the complete site directory or return home.',
          'card'
        );
      ...
      appendText(
        fallback,
        'p',
        'Use the complete site directory to continue.'
      );
      const link = document.createElement('a');
      link.className = 'btn secondary';
